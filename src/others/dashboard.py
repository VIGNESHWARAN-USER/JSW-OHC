...
def Dashboard(connection,cursor,accessLevel):
    ...
    r0c1, r0c2, r0c3, r0c4 = st.columns([3,3,3,3])
    with r0c1:
    ...
    with r0c2:
        ...
        with rr1:
            st.write("From")
        with rr2:
            date_val = st.date_input("From", label_visibility='collapsed')
    with r0c3:
        rr1, rr2 = st.columns([0.6,6])
        with rr1:
            st.write("To")
        with rr2:
            date_val = st.date_input("To", label_visibility='collapsed')
    with r0c4:
    ...
    with st.container(border=1, height=700):
        ...
        with rc1:
            st.session_state.optFilter = option_menu(None, options=["Preventive", "Curative"], icons=['a','a'], orientation='horizontal')
        with rc2:
        ...
        with st.container(border=1, height=500):
            if st.session_state.optFilter == "Preventive":
                ...
                st.pyplot(fig)
            if st.session_state.optFilter == "Curative":
